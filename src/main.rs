...
impl AppState {
    ...
    pub fn new(private_key_location: &str) -> Self {      
        Self { 
            rsa_key_pair : Secret::rsa_keypair_from_file(private_key_location).expect("Cannot read RSA keypair"),
        }
    }
}
...
async fn main() -> std::io::Result<()> {
    let args = clap::App::new("oidc-token-test-service")
       ...
       .arg(clap::Arg::with_name("keyfile")
            .help("Location of the RSA DER keypair as a file")
            .required(false)
            .index(1))
       .get_matches(); 

    let keyfile = args.value_of("keyfile").unwrap_or("./static/private_key.der").to_owned();
    ...
    HttpServer::new( move|| {
        App::new()
            ...
            .service(
                web::resource("/token")
                    .route(web::post().to(token::create_token))
            )
            .service(
            ...
            .service(
                web::resource("/keys")
                    .route(web::get().to(discovery::keys))   
            )
            .service(web::resource("/health")
                    .route(web::get().to(checks::check))
            )
    })
    .bind("0.0.0.0:8080")?
    .run()
...
mod tests {
    ...
    #[test]
    #[should_panic(expected = "Cannot read RSA keypair: IOError(Os { code: 2, kind: NotFound, message: \"No such file or directory\" })")]
